...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/ShylyPP/V2bX/dev_new/V2bX-script-master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/ShylyPP/V2bX/dev_new/V2bX-script-master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    local shell_url="https://raw.githubusercontent.com/ShylyPP/V2bX/dev_new/V2bX-script-master/V2bX.sh"
    if [[ x"${release}" == x"alpine" ]]; then
